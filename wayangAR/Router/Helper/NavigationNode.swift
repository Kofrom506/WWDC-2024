...
indirect enum NavigationNode<Screen: ScreenProtocol, ScreenView: View>: View {
    case view(ScreenView, context: RouterContext<Screen>, pushing: NavigationNode<Screen, ScreenView>, stack: Binding<[RouterContext<Screen>]>, index: Int)
    case end
            ...
            view
                .background(
                    NavigationLink(
                        ...
                        isActive: pushBinding.wrappedValue ? pushBinding : fullCoverBinding,
    ...
    var body: some View {
        if routerContext?.screen.embedInNavView ?? false {
            NavigationView {
                viewBody
